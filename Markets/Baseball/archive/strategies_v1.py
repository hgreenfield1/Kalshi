...
class PriceDipStrategy(BaseStrategy):
    """
    Kalshi-price-only strategy (v6.0.0).
    Buys when ask price drops >=15 points to below 10. Holds to maturity.
    Makes no MLB Stats API calls.
    """

    _version = "v6.0.0"
    _prediction_model_version = "none"
    ...
    def get_data_requirements(self):
        return [DataRequirement(
            data_key="kalshi",
            loader_class="Markets.Baseball.kalshi_data_loader.KalshiDataLoader",
            params={}
        )]
    ...
    def on_timestep(self, context):
        ...
        # Track peak ask price seen so far
        mid = context.mid_price
        if mid is not None:
            if self.peak_price is None or mid > self.peak_price:
                self.peak_price = mid
        ...
        if self.last_trade_time:
            minutes_since = date_helpers.minutes_between_timestamps(
                self.last_trade_time, context.timestamp
            )
            if minutes_since < self.trade_cooldown_minutes:
                return []

        # Buy signal: price dropped >=15 from peak and ask is now <10
        drop = self.peak_price - ask_price
        positions = context.portfolio_snapshot['positions']

        if drop >= 15 and ask_price < 10 and positions < self.position_limit:
            self.last_trade_time = context.timestamp
            return [Order(OrderSide.BUY, 1, ask_price)]
